...
#==================
with(df2, plot(as.numeric(as.character(Global_active_power)), type="l", ylab='Global Active Power (kilowatts)', xaxt="n", xlab=""))
with(df2,axis(1, at=c(1,nrow(df2)/2,nrow(df2)),  labels=c("Thurs","Fri", "Sat"),las=2))

#==================
with(df2, plot(as.numeric(as.character(Voltage)), type="l", ylab='Voltage', xaxt="n", xlab=""))
with(df2,axis(1, at=c(1,nrow(df2)/2,nrow(df2)),  labels=c("Thurs","Fri", "Sat"),las=2))
...
legend('topright', c("Sub_metering_1", "Sub_metering_2","Sub_metering_3") , 
       lty=1, col=c('black', 'red', 'blue'), bty='n', pt.cex = 1, cex=.75)


#==================
with(df2, plot(as.numeric(as.character(Global_reactive_power)), type="l", ylab='Global_reactive_power', ylim=c(0.0,0.5), xaxt="n", xlab="datetime"))
with(df2,axis(1, at=c(1,nrow(df2)/2,nrow(df2)),  labels=c("Thurs","Fri", "Sat"),las=2))
